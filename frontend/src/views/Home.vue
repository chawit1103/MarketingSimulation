<template>
  <div class="home-container">
    ...
    <nav class="navbar" :style="s.navbar">
      <div class="nav-brand" :style="s.navBrand">{{ $t('home.brand') }}</div>
      <div class="nav-links" :style="s.navLinks">
        <router-link to="/campaigns" class="nav-link" :style="s.navLink">Campaigns</router-link>
        <router-link to="/settings" class="nav-link" :style="s.navLink">Settings</router-link>
    ...
    <div class="main-content" :style="s.mainContent">
      ...
      <section class="hero-section" :style="s.heroSection">
        <div class="hero-left" :style="s.heroLeft">
          <div class="tag-row" :style="s.tagRow">
            <span class="orange-tag" :style="s.orangeTag">{{ $t('home.tag') }}</span>
            <span class="version-text" :style="s.versionText">{{ $t('home.version') }}</span>
          </div>

          <h1 class="main-title" :style="s.mainTitle">
            {{ $t('home.heroLine1') }}<br>
            <span class="gradient-text" :style="s.gradientText">{{ $t('home.heroLine2') }}</span>
          </h1>

          <div class="hero-desc" :style="s.heroDesc">
            <p :style="s.heroDescP">
              {{ $t('home.description1') }} <span :style="s.highlightBold">{{ $t('home.description2') }}</span> {{ $t('home.description3') }} <span :style="s.highlightOrange">{{ $t('home.description4') }}</span> {{ $t('home.description5') }} <span :style="s.highlightCode">{{ $t('home.description6') }}</span> {{ $t('home.description7') }}
            </p>
      ...
      <section class="dashboard-section" :style="s.dashboardSection">
        ...
        <div class="left-panel" :style="s.leftPanel">
          ...
          <h2 class="section-title" :style="s.sectionTitle">{{ $t('home.ready') }}</h2>
          ...
          <div class="metrics-row" :style="s.metricsRow">
            <div class="metric-card" :style="s.metricCard">
              <div class="metric-value" :style="s.metricValue">{{ $t('home.metricProviders') }}</div>
              <div class="metric-label" :style="s.metricLabel">{{ $t('home.metricProvidersLabel') }}</div>
            </div>
            <div class="metric-card" :style="s.metricCard">
              <div class="metric-value" :style="s.metricValue">{{ $t('home.metricLanguages') }}</div>
              <div class="metric-label" :style="s.metricLabel">{{ $t('home.metricLanguagesLabel') }}</div>
            </div>
          </div>

          <div class="steps-container" :style="s.stepsContainer">
            <div class="steps-header" :style="s.stepsHeader">
               <span :style="s.diamondIcon">◇</span> {{ $t('home.workflowTitle') }}
            </div>
          ...
          <div class="console-box" :style="s.consoleBox">
            <div :style="s.consoleSection">
              <div class="console-header" :style="s.consoleHeader">
                ...
                <span>{{ $t('home.consoleSupported') }}</span>
              </div>
              <div
                :style="s.uploadZone"
              ...
              >
                ...
                <div v-if="files.length === 0" :style="s.uploadPlaceholder">
                  ...
                  <div :style="s.uploadTitle">{{ $t('home.uploadTitle') }}</div>
                  <div :style="s.uploadHint">{{ $t('home.uploadHint') }}</div>
                </div>
            ...
            <div :style="s.consoleSection">
              <div class="console-header" :style="s.consoleHeader">
                <span>>_ 02 / {{ $t('home.consoleSimPrompt') }}</span>
              </div>
              <div :style="s.inputWrapper">
                <textarea v-model="formData.simulationRequirement" :style="s.codeInput" :placeholder="$t('home.simPromptPlaceholder')" rows="6" :disabled="loading"></textarea>
...
const s = reactive({
  ...
  navLink: { color: '#999', textDecoration: 'none', fontFamily: mono, fontSize: '0.85rem', marginRight: '20px', transition: 'color 0.2s' },
