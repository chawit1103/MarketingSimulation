  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('process.step3InfoPlaza') }}</span>
          ...
          <div class="platform-stats">
            <span class="stat">
              <span class="stat-label">{{ $t('process.step3Round') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('process.step3ElapsedTime') }}</span>
              <span class="stat-value mono">{{ twitterElapsedTime }}</span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('process.step3Acts') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_actions_count || 0 }}</span>
            </span>
          </div>
          <!-- Available Actions Tooltip -->
          <div class="actions-tooltip">
            <div class="tooltip-title">{{ $t('process.step3AvailableActions') }}</div>
            <div class="tooltip-actions">
              <span class="tooltip-action">{{ $t('process.step3ActionPost') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionLike') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionRepost') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionQuote') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionFollow') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionIdle') }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('process.step3TopicCommunity') }}</span>
          ...
          <div class="actions-tooltip">
            ...
            <div class="tooltip-actions">
              <span class="tooltip-action">{{ $t('process.step3ActionPost') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionComment') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionLike') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionDislike') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionSearch') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionTrend') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionFollow') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionMute') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionRefresh') }}</span>
              <span class="tooltip-action">{{ $t('process.step3ActionIdle') }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('process.step3Starting') : $t('process.step3StartGeneratingReport') }} 
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">{{ $t('process.step3TotalEvents') }}: <span class="mono">{{ allActions.length }}</span></span>
      ...
      <div class="timeline-feed">
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'QUOTE_POST'">
                  ...
                  <div v-if="action.action_args?.original_content" class="quoted-block">
                    <div class="quote-header">
                      ...
                      <span class="quote-label">@{{ action.action_args.original_author_name || $t('process.user') }}</span>
                    </div>
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">{{ $t('process.step3RepostedFrom') }} @{{ action.action_args?.original_author_name || $t('process.user') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">{{ $t('process.step3LikedPost', { user: action.action_args?.post_author_name || $t('process.user') }) }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>{{ $t('process.step3ReplyToPost', { id: action.action_args.post_id }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">{{ $t('process.step3SearchQuery') }}:</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">{{ $t('process.step3FollowedUser', { user: action.action_args?.target_user || action.action_args?.user_id || $t('process.user') }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? $t('process.step3UpvotedPost') : $t('process.step3DownvotedPost') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    ...
                    <span class="idle-label">{{ $t('process.step3ActionSkipped') }}</span>
                  </div>
                </template>
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          ...
          <span>{{ $t('process.step3WaitingActions') }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">{{ $t('process.step3SimulationMonitor') }}</span>
...
const formatElapsedTime = (currentRound) => {
  if (!currentRound || currentRound <= 0) return t('process.zeroTime')
  const totalMinutes = currentRound * props.minutesPerRound
  const hours = Math.floor(totalMinutes / 60)
  const minutes = totalMinutes % 60
  return t('process.elapsedHoursMinutes', { hours, minutes })
}
...
const getActionTypeLabel = (type) => {
  const labels = {
    'CREATE_POST': t('process.step3ActionPost'),
    'REPOST': t('process.step3ActionRepost'),
    'LIKE_POST': t('process.step3ActionLike'),
    'CREATE_COMMENT': t('process.step3ActionComment'),
    'LIKE_COMMENT': t('process.step3ActionLike'),
    'DO_NOTHING': t('process.step3ActionIdle'),
    'FOLLOW': t('process.step3ActionFollow'),
    'SEARCH_POSTS': t('process.step3ActionSearch'),
    'QUOTE_POST': t('process.step3ActionQuote'),
    'UPVOTE_POST': t('process.step3ActionUpvote'),
    'DOWNVOTE_POST': t('process.step3ActionDownvote')
  }
  return labels[type] || type || t('process.unknown')
}
